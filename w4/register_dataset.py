...
from utils.dataset_gestions import load_labels
from utils.json_loader import saveData, loadData
...
def register_city_challenge(txt_path):
    ...
    # If json file does not exist, create it
        ...
        for sequence_name in tqdm(txt_lines, desc=f"registering {txt_path.split('.')[0].split('/')[1]} set"):
            ...
            gt_detections = load_labels(os.path.join('frames', sequence_name), 'gt.txt')        # Get the ground truth
